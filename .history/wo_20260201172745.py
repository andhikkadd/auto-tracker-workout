...
def update_active_counter(features):
    if STATE["active"] == "Bicep Curl":
        ...
        # _, R["stage"], R["reps"], R["down_ref"], R["up_ref"], R["down_hold"], R["up_hold"] = process_arm(
        #     features["ang_R"],
        #     features["r_shoulder"], features["r_elbow"], features["r_wrist"],
        #     R["stage"], R["reps"],
        #     R["down_ref"], R["up_ref"],
        #     R["down_hold"], R["up_hold"],
        #     STATE["HOLD_N"], STATE["ALPHA"], STATE["MARGIN"], STATE["v"]
        # )
...
def render_ui(frame, features):   
    ...
    if STATE["active"] == "Bicep Curl":
        total_reps = (STATE["curl"]["arms"]["L"]["reps"]) + (STATE["curl"]["arms"]["R"]["reps"])
        if total_reps > STATE["curl"]["prev_total"]:
            winsound.Beep(3000, 120) 
            STATE["curl"]["prev_total"] = total_reps
            
        reps_L = STATE["curl"]["reps"]
        reps_R = STATE["curl_R"]["reps"]
        ...
        total_reps_pullup = STATE["pull"]["reps"]
        if total_reps_pullup > STATE["prev_total_pull"]:
            winsound.Beep(3000, 120)
            STATE["prev_total_pull"] = total_reps_pullup
    ...
    if features["l_index"].visibility > STATE["v"]:
        x, y = features["li_px"]
        touch = touch or inside(x, y, BTN_X, BTN_Y, BTN_W, BTN_H)
    ...
    if STATE["reset_hold"] >= STATE["RESET_HOLD_N"] and STATE["reset_armed"] == True:
        STATE["curl_L"]["reps"] = STATE["curl_R"]["reps"] = STATE["prev_total_curl"] = 0
        STATE["pull"]["reps"] = STATE["prev_total_pull"] = 0
