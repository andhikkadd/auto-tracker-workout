...
def process_pull_up(features, pull_state):
    lw = features["l_wrist"]
    rw = features["r_wrist"]
    ...
    # === detect bottom ===
    if pull_state["bottom_ref"] is None or delta > pull_state["bottom_ref"] - MARGIN:
        pull_state["bottom_hold"] += 1
    else:
        pull_state["bottom_hold"] = 0

    if pull_state["bottom_hold"] >= HOLD:
        pull_state["bottom_ref"] = (
            delta if pull_state["bottom_ref"] is None
            else pull_state["bottom_ref"] * (1 - ALPHA) + delta * ALPHA
        )
        pull_state["stage"] = "down"

    # === detect top ===
    if pull_state["top_ref"] is None or delta < pull_state["top_ref"] + MARGIN:
        pull_state["top_hold"] += 1
    else:
        pull_state["top_hold"] = 0

    if pull_state["top_hold"] >= HOLD:
        pull_state["top_ref"] = (
            delta if pull_state["top_ref"] is None
            else pull_state["top_ref"] * (1 - ALPHA) + delta * ALPHA
        )

        if pull_state["stage"] == "down":
            pull_state["reps"] += 1
            pull_state["stage"] = "up"
...
def update_active_counter(features):
    if STATE["active"] == "Bicep Curl":
        ...
        # process_bicep_curl(features, STATE["curl_L"], STATE["curl_R"])
        L = STATE["curl_L"]
        R = STATE["curl_R"]

        _, L["stage"], L["reps"], L["down_ref"], L["up_ref"], L["down_hold"], L["up_hold"] = process_arm(
            features["ang_L"],
            features["l_shoulder"], features["l_elbow"], features["l_wrist"],
            L["stage"], L["reps"],
            L["down_ref"], L["up_ref"],
            L["down_hold"], L["up_hold"],
