...
def process_pull_up(features,):
    ...
    # === detect bottom ===
    if STATE ["bottom_ref"] is None or delta > STATE ["bottom_ref"] - MARGIN:
        STATE ["bottom_hold"] += 1
    else:
        STATE ["bottom_hold"] = 0

    if STATE ["bottom_hold"] >= HOLD:
        STATE ["bottom_ref"] = (
            delta if STATE ["bottom_ref"] is None
            else STATE ["bottom_ref"] * (1 - ALPHA) + delta * ALPHA
        )
        STATE ["stage"] = "down"

    # === detect top ===
    if STATE ["top_ref"] is None or delta < STATE ["top_ref"] + MARGIN:
        STATE ["top_hold"] += 1
    else:
        STATE ["top_hold"] = 0

    if STATE ["top_hold"] >= HOLD:
        STATE ["top_ref"] = (
            delta if STATE ["top_ref"] is None
            else STATE ["top_ref"] * (1 - ALPHA) + delta * ALPHA
        )
